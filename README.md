...
当前稳定固件：**v1.7-memorybread**

v1.7 起，传输模式会始终开启 `MemoryBread-Setup` 热点，同时尝试连接设备中保存的 Wi-Fi。手机连接该热点后可稳定访问 `http://192.168.4.1`；即使办公室或公共 Wi-Fi 开启了设备隔离，也不会被不可访问的局域网 IP 卡住。
